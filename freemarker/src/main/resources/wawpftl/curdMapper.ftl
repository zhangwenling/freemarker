 ...
 <insert id="${table.shortTableName?cap_first}_insert" parameterType="${table.shortTableName?cap_first}">
  <selectKey resultType="long" keyProperty="id" order="BEFORE">SELECT SEQ_${table.sequence}.nextval FROM DUAL </selectKey>
     INSERT INTO ${table.tableName} (
        ...
        VALUES (
                 <#list table.columnList as being>
                 	<#if being.isEnum>#${"{" + being.shortColumnName}, jdbcType=${being.jdbcType}, typeHandler=${being.packageStr + ".handler."}<#if being.shortColumnName=="state">YesOrNoHandler<#else>${being.enumClassName}Handler</#if>}<#else>#${"{" + being.shortColumnName}, jdbcType=${being.jdbcType}}</#if><#if being_has_next>,</#if>
                 </#list>
                )
 </insert>
 <!-- 删除配置 --> 
 <delete id="${table.shortTableName?cap_first}_delete" parameterType="map">
	DELETE ${table.tableName} T
...
SELECT TT.*,ROWNUM FROM (SELECT T.* FROM ${table.tableName} T
